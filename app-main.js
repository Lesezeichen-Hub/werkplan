...
document.querySelector('#openHelp')?.addEventListener('click', openHelp);
document.querySelector('#closeHelp')?.addEventListener('click', closeHelp);
document.querySelector('#helpDialog')?.addEventListener('keydown', trapHelpTab);
document.querySelector('#helpDialog')?.addEventListener('pointerdown', event => { if (event.target.id === 'helpDialog') closeHelp(); });
document.querySelector('#helpSearch')?.addEventListener('input', filterHelp);
document.addEventListener('keydown', event => {
  const helpDialog = document.querySelector('#helpDialog');
  if (event.key === 'Escape' && !helpDialog.hidden) { event.preventDefault(); event.stopImmediatePropagation(); closeHelp(); }
  if (event.key === 'F1') { event.preventDefault(); event.stopImmediatePropagation(); openHelp(); }
}, true);
