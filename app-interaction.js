...
function renderObjectList() {
  ...
  logicalObjectEntries().forEach((entry, index) => {
    ...
    const toolId = members.find(item => item.workshopTool || item.woodTool)?.workshopTool || members.find(item => item.woodTool)?.woodTool; const groupName = toolId ? hardwareToolNames[toolId] || woodToolNames[toolId] || toolId : members.length > 1 ? 'Gruppe' : object.name || toolNames[object.type] || object.type;
...
function handlePointerMove(event) {
  ...
  if (isGeneratedTool() && state.tool !== 'freihandkurve') { clearPreview(); createGeneratedGeometry(pointerStart, point).filter(Boolean).forEach(object => renderObject(object, previewLayer)); }
}
function handlePointerUp(event) {
  ...
  if (isGeneratedTool()) { if (!pointerStart) return; const point = eventPoint(event, false, pointerStart); if (distance(pointerStart, point) >= 3) addGeneratedObjects(createGeneratedGeometry(pointerStart, point)); pointerStart = null; clearPreview(); return; }
...
function setTool(tool) { state.tool = tool; document.querySelectorAll('.tool-button').forEach(button => button.classList.toggle('active', button.dataset.tool === tool || button.dataset.planned === tool || button.dataset.hardware === tool)); document.querySelector('#toolHint').textContent = `${toolNames[tool] || woodToolNames[tool] || hardwareToolNames[tool] || tool} aktiv`; document.querySelector('#lineLengthPanel').hidden = !(['line', 'dimension', 'rect', 'circle', 'semicircle', 'ellipse', 'ellipseArc', 'slot', 'polygon'].includes(tool) || isGeneratedTool(tool)); document.querySelector('#profileEditorPanel').hidden = tool !== 'profilEditor'; updateLiveAngle(null, null); clearPreview(); polylinePoints = []; }
